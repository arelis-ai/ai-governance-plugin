...
decision = await evaluate_pre_invocation_gate(
    source=arelis,  # ArelisInstance, ArelisPlatform, or GovernanceGateEvaluator
...
```python
from arelis import with_governance_gate, WithGovernanceGateOptions, EvaluatePreInvocationGateInput
...
```python
# Version management
await platform.governance.policies.createVersion(policy_id, {...})
await platform.governance.policies.listVersions(policy_id)
await platform.governance.policies.activateVersion(policy_id, {"version": "v2"})

# Lifecycle
await platform.governance.policies.transition(policy_id, {"status": "active"})
await platform.governance.policies.rollback(policy_id, {"version": "v1"})

# Testing
await platform.governance.policies.simulate(policy_id, {"checkpoint": {...}})
await platform.governance.policies.bulkSimulate({"policies": [...], "checkpoint": {...}})
await platform.governance.policies.impact(policy_id, {"changes": {...}})
```
...
async def manual_pre_invocation_gate(
    ...
    eval_result = await platform.governance.evaluatePolicy({
        "runId": run_id,
        "checkpoint": {
            "content": {
