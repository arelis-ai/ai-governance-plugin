...
- **TypeScript: Always `await` platform calls** — use `.catch()` to swallow errors in serverless runtimes
- **Python: Platform calls are synchronous** — do NOT use `await`; use try/except to swallow errors
